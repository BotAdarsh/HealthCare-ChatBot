...
model = AutoModel.from_pretrained("question_extractor_model")

def vectorize_query(query, tokenizer, model):
    encoded_question= tokenizer.encode(query)
    
    v_q=model(encoded_question)
    return v_q
...
if prompt := st.chat_input("Ask a medical question?"):
    ...
    gen_response=""
    responses = display_search_results(prompt,tokenizer,model,0.5)
    if len(responses)==0:
